...
def evaluate(x_op, y_op, init_size=30, iteration=18):
    ...
    try:
        ...
        print('error! use small cluster size')
        M, C = build_kmeans(dist_mat, init_num=int(init_size / 2))
        ...
        for k in range(fc_output[M].shape[0]):
            ...
            if temp_index[0].shape[0] == 0:
                ...
                dist = pairwise_distances(
                    fc_output[M], temp, metric='euclidean')
                cluster_label = np.argmin(dist, axis=0)
                Flag = True
                init_size = np.maximum(1, int(init_size / 2))
...
if __name__ == '__main__':
    ...
    # model_dir = './model/fine_tune100.pth'
    model = Net()
    ...
    x_valid = torch.FloatTensor(x_valid)[0:1000]
    y_valid = torch.LongTensor(y_valid)[0:1000]
