...
def train_model(file_path='./model'):
    ...
    train_loader, valid_loader = dp.load_imageclef_train(
        '../data/', 'c', train_batch, 'src')
    test_loader = dp.load_imageclef_test( 
       '../data/', 'p', train_batch, 'tar')

    # train_loader, valid_loader = dp.load_imageclef_train('../data/', 'p', train_batch, 'src')
    ...
    optimizer = optim.SGD(net.fc.parameters(), lr=1e-3, momentum=0.9)
    net.train()
