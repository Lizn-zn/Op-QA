...
class MyClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X, y=None):
        X = torch.tensor(X)
        softmaxes = F.softmax(X, dim=1)
        softmaxes = softmaxes.detach().numpy()
        return np.argmax(softmaxes, axis=1)
...
def calibrated_profit_curve(model, calibrated_model, x_test, y_test, interval=0.05):
    softmaxes = F.softmax(model(o_test))
    confidences, predictions = torch.max(softmaxes, 1)
    confidences = calibrated_model.predict(confidences.detach().numpy())
    predictions = predictions.detach().numpy()
    y_test = y_test.detach().numpy()
...
if __name__ == '__main__':
    ...
    x_train, _ = torch.max(F.softmax(model.fc3(x_op)), 1)
    x_train = x_train.detach().numpy()
    y_train = y_op.detach().numpy()
    calibrator.fit(x_train, y_train)
    print('platt scaling results: ')
    calibrated_profit_curve(model, calibrator, x_test, y_test, interval=0.05)
