...
def conf_build(model, x, center_):
    ...
    for k in range(center_.shape[0]):
        ...
        import Optim
        opt = Optim.adam
        # kernel = ConstantKernel(1) * RationalQuadratic(length_scale=length_scale)
...
def opc_predict(model, clf, x_new, center_):
    ...
    softmaxes = F.softmax(model.classifier(torch.Tensor(x_new).cuda()), dim=1)
    confidences, predictions = torch.max(softmaxes, 1)
    confidences = confidences.cpu().detach().numpy()
    ...
    for k in range(x_new.shape[0]):
        ...
        gp = clf[i]
        # c, std1 = gp1.predict(temp, return_std=True)
        # print(confidences[k], c)
        c = confidences[k] 
        r = gp.predict(temp)
        pred.append(
            np.clip(c + r, 1e-3, 1 - 1e-3))
