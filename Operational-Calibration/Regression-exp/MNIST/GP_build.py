...
def conf_build(model, x, center_):
    ...
    for k in range(center_.shape[0]):
        ...
        kernel = ConstantKernel(
            1) * RBF(length_scale=length_scale, length_scale_bounds=(1e-5, 1e5)) + ConstantKernel(
            1) * (DotProduct(sigma_0=sigma_0) ** 2)
...
def ratio_build(model, x, x_select, y_select, select_index, center_):
    ...
    # get conf ratio
    softmaxes = F.softmax(model.fc3(x_select), dim=1)
    confidences, predictions = torch.max(softmaxes, 1)
    predictions = predictions.detach().numpy()
    confidences = confidences.detach().numpy()
    confidences = np.clip(confidences, 1e-3, 1 - 1e-3)
...
def opc_predict(model, clf, x_new, center_):
    ...
    softmaxes = F.softmax(model.fc3(x_new), dim=1)
    ...
    for k in range(x_new.shape[0]):
        ...
        std1 = 0
        # pred.append(
        # np.clip(np.exp(c + r), 1e-3, 1 - 1e-3))
        pred.append(np.clip(c + r, 1e-3, 1 - 1e-3))
        std.append(std1)
